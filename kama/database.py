...
CREATE TABLE `permissions` (
    ...
    with get_database_context() as cursor:
        ...
        cursor.execute('INSERT INTO `entities` (entity_uuid, kind, name) VALUES(UNHEX(%s), %s, %s)',
                (role_uuid.hex(), 'role', 'root'))
        cursor.execute('INSERT INTO `entities` (entity_uuid, kind, name) VALUES(UNHEX(%s), %s, %s)',
                (user_uuid.hex(), 'user', 'root'))
        cursor.execute('INSERT INTO `links` (link_uuid, from_uuid, to_uuid) VALUES(UNHEX(%s), UNHEX(%s), UNHEX(%s))',
                (link_uuid.hex(), role_uuid.hex(), user_uuid.hex()))
...
class Entity(object):
    ...
    def get_all(cls):
        with get_database_context() as cursor:
            ...
            return [cls(bytes.fromhex(uuid), kind, name) for uuid, kind, name in cursor]

    def get(self):
        with get_database_context() as cursor:
            cursor.execute('SELECT kind, name FROM entities WHERE entity_uuid=UNHEX(%s)', (self.uuid.hex(),))
            row = cursor.fetchone()
            if row is None:
    ...
    def delete(self, context=None):
        with get_database_context() as cursor:
            cursor.execute('DELETE FROM entities WHERE entity_uuid=UNHEX(%s)', (self.uuid.hex(),))
            cursor.execute('DELETE FROM attributes WHERE entity_uuid=UNHEX(%s)', (self.uuid.hex(),))
            cursor.execute('DELETE FROM links WHERE to_uuid=UNHEX(%s) OR from_uuid=UNHEX(%s)', (self.uuid.hex(), self.uuid.hex()))
            cursor.execute('DELETE FROM permissions WHERE entity_uuid=UNHEX(%s) OR role_uuid=UNHEX(%s)', (self.uuid.hex(), self.uuid.hex()))
    ...
    def set_name(self, name, context=None):
        with get_database_context() as cursor:
            cursor.execute('UPDATE entities SET name=%s WHERE entity_uuid=UNHEX(%s)', (name, self.uuid.hex()))
            self.name = name
        if isinstance(context, RequestContext):
    ...
    def delete_attributes(self, key, context=None):
        with get_database_context() as cursor:
            cursor.execute('DELETE FROM attributes WHERE entity_uuid=UNHEX(%s) AND `key`=%s', (self.uuid.hex(), key))
    ...
    def attributes(self, key=None, context=None):
        with get_database_context() as cursor:
            if key is not None:
                cursor.execute('SELECT HEX(attribute_uuid), `key`, `value` FROM attributes WHERE entity_uuid=UNHEX(%s) AND `key`=%s', (self.uuid.hex(), key))
            else:
                cursor.execute('SELECT HEX(attribute_uuid), `key`, `value` FROM attributes WHERE entity_uuid=UNHEX(%s)', (self.uuid.hex(),))
            if isinstance(context, RequestContext):
                audit_log.info('%s entity.read_attribute %s key=%s', context.user, self, key)
            return [Attribute(bytes.fromhex(uuid), key, value) for uuid, key, value in cursor]
    ...
    def delete_link(self, to_entity, context=None):
        with get_database_context() as cursor:
            cursor.execute('DELETE FROM links WHERE from_uuid=UNHEX(%s) AND to_uuid=UNHEX(%s)', (self.uuid.hex(), to_entity.uuid.hex()))
            if cursor.rowcount == 0:
    ...
    def links_from(self, context=None):
        links = []
        with get_database_context() as cursor:
            cursor.execute('SELECT HEX(links.link_uuid), HEX(links.to_uuid), entities.kind, entities.name FROM links, entities WHERE from_uuid=UNHEX(%s) AND entities.entity_uuid=links.to_uuid', (self.uuid.hex(),))
    ...
    def links_to(self, context=None):
        links = []
        with get_database_context() as cursor:
            cursor.execute('SELECT HEX(link_uuid), HEX(from_uuid), entities.kind, entities.name FROM links, entities WHERE to_uuid=UNHEX(%s) AND entities.entity_uuid=links.from_uuid', (self.uuid.hex(),))
            for link_uuid, from_uuid, from_entity_kind, from_entity_name in cursor:
                link_uuid = bytes.fromhex(link_uuid)
                from_uuid = bytes.fromhex(from_uuid)
    ...
    def delete_permission(self, role, name, context=None):
        with get_database_context() as cursor:
            cursor.execute('DELETE FROM permissions WHERE entity_uuid=UNHEX(%s) AND role_uuid=UNHEX(%s) AND name=%s', (self.uuid.hex(), role.uuid.hex(), name))
        if isinstance(context, RequestContext):
    ...
    def permissions(self, context=None):
        permissions = []
        with get_database_context() as cursor:
            cursor.execute('SELECT HEX(permission_uuid), HEX(role_uuid), entities.kind, entities.name, permissions.name FROM permissions, entities WHERE permissions.entity_uuid=UNHEX(%s) AND entities.entity_uuid=permissions.role_uuid', (self.uuid.hex(),))
            for permission_uuid, role_uuid, role_kind, role_name, name in cursor:
                permission_uuid = bytes.fromhex(permission_uuid)
                role_uuid = bytes.fromhex(role_uuid)
    ...
    def has_permission(self, entity, name):
        with get_database_context() as cursor:
            cursor.execute('SELECT HEX(permissions.permission_uuid) FROM permissions, entities AS role, entities AS users, links WHERE permissions.entity_uuid=UNHEX(%s) AND permissions.name=%s AND role.entity_uuid=permissions.role_uuid AND links.from_uuid=role.entity_uuid AND links.to_uuid=users.entity_uuid AND users.entity_uuid=UNHEX(%s)', (entity.uuid.hex(), name, self.uuid.hex()))
            if cursor.rowcount > 0:
                return True
            else:
                return False
    ...
    def _create(cls, entity_uuid, key, value):
        with get_database_context() as cursor:
            ...
            cursor.execute('INSERT INTO attributes(attribute_uuid, entity_uuid, `key`, `value`) VALUES(UNHEX(%s), UNHEX(%s), %s, %s)',
                    (attribute_uuid.hex(), entity_uuid.hex(), key, value))
            return cls(attribute_uuid, entity_uuid, key, value)

    def get(self):
        with get_database_context() as cursor:
            cursor.execute('SELECT HEX(entity_uuid), `key`, `value` FROM attributes WHERE attribute_uuid=UNHEX(%s)', (self.uuid.hex(),))
            row = cursor.fetchone()
            if row is None:
                raise AttributeNotFoundException(self.uuid)

            uuid, key, value = row
            self.entity_uuid = bytes.fromhex(uuid)
            self.key = key
            self.value = value
...
class Link(object):
    ...
    def _create(cls, from_entity, to_entity):
        with get_database_context() as cursor:
            link_uuid = generate_uuid()
            cursor.execute('INSERT INTO links (link_uuid, from_uuid, to_uuid) VALUES(UNHEX(%s), UNHEX(%s), UNHEX(%s))',
                    (link_uuid.hex(), from_entity.uuid.hex(), to_entity.uuid.hex()))
            return cls(link_uuid, from_entity, to_entity)

    def get(self):
        with get_database_context() as cursor:
            cursor.execute('SELECT HEX(from_uuid), HEX(to_uuid) FROM links WHERE link_uuid=UNHEX(%s)', (self.uuid.hex(),))
            row = cursor.fetchone()
            if row is None:
    ...
    def _create(cls, role, entity, name):
        with get_database_context() as cursor:
            ...
            cursor.execute('INSERT INTO permissions (permission_uuid, role_uuid, entity_uuid, name) VALUES(UNHEX(%s), UNHEX(%s), UNHEX(%s), %s)',
                    (permission_uuid.hex(), role.uuid.hex(), entity.uuid.hex(), name))
            return cls(permission_uuid, role, entity, name)
    
    def get(self):
        with get_database_context() as cursor:
            cursor.execute('SELECT HEX(role_uuid), HEX(entity_uuid), name FROM permissions WHERE permission_uuid=UNHEX(%s)', (self.uuid.hex(),))
            row = cursor.fetchone()
            if row is None:
                raise PermissionNotFoundException(self.uuid)
            role_uuid, entity_uuid, self.name = row
            role_uuid = bytes.fromhex(role_uuid)
            entity_uuid = bytes.fromhex(entity_uuid)
